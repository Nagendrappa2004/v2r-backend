...
router.post("/create-order", authUser, async (req, res) => {
  try {
    ...
    if (amount != null && Math.abs(Number(amount) - expectedTotal) > 1) {
      return res.status(400).json({
        error: "Order total mismatch. Please refresh checkout.",
        expectedTotal
      });
    }
...
router.post("/verify", authUser, async (req, res) => {
  try {
    ...
    const { name, phone, address, items, email } = orderData;
    ...
    let notifications = {};
    try {
      notifications = await notifyOrderPlaced(newOrder);
    } catch (e) {
      console.warn("Order notifications failed:", e.message);
    }

    res.status(201).json({ message: "Order placed", order: newOrder, notifications });
  } catch (err) {
